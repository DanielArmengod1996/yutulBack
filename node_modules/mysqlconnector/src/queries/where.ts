        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            switch (v) {
                ...
                default:
                    ...
                    if (vStr.indexOf("!=") === 0) {
                        const actual = vStr.substr(2, vStr.length).trim();
                        this.sql += `${s}${keys[i]} != ${actual}${n}`;
                    } else if (vStr.indexOf(">") === 0 || vStr.indexOf("<") === 0) {
                        const symb = vStr.substr(0, 1).trim();
                        const val = vStr.substr(1, vStr.length).trim();
                        this.sql += `${s}${keys[i]} ${symb} ${val}${n}`;
                    } else {
