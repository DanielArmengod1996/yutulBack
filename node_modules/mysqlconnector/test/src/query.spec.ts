...
describe("Query", () => {
    ...
    it("Should produce the same query", () => {
        ...
        const q4 = Select.Table("user")
            .where({id: "!= 1"}).toString();

        const q6 = Select.Table("user")
            .where({id: "> 1"}).toString();

        expect(q1).to.be.equals(q2);
        expect(q1).to.be.equals(q3);
        expect(q4).to.be.equals(Select.Table("user").where("id != 1").toString());
        expect(q6).to.be.equals(Select.Table("user").where("id > 1").toString());
    });
});
