...
app.post("/setVideo", (req, res) => {
  ...
  callout.then(function(finalResponse) {
    ...
    const params = [
      ...
      finalResponse.tags,
      finalResponse.url_video,
      finalResponse.url_image,
      0,
      finalResponse.id_usuario
    ];
    const query =
      "INSERT INTO videos (nombre, description, tags, localizacionVideo, localizacionImage, likes, id_usuario ) VALUES ( ?, ?, ?, ?, ?, ?, ? )";

    doQuery(query, params).then(results => {
      bodyResponse = JSON.stringify(
        wrapper.instanceStatusRegistered("OK", new Array())
      );
...
app.post("/joinSession", (req, res) => {
  ...
  callout.then(function(finalResponse) {
    ...
    doQuery(query, params).then(results => {
      ...
      var classResponse =
        results.length > 0
          ? JSON.stringify(wrapper.instanceSessionResponse(results[0].id))
...
/**
 *  @method Method that is gona to register an User into the database
 */
  ...
  callout.then(function(finalResponse) {
    executeRegisterValidations(finalResponse).then(function(finalValidation) {
      ...
      if (finalValidation.length > 0) {
        bodyResponse = JSON.stringify(
          wrapper.instanceStatusRegistered("KO", finalValidation)
        );
        console.log(bodyResponse);
        res.send(bodyResponse);
      } else {
        const params = [
          finalResponse.name,
          finalResponse.lastname,
          finalResponse.nick,
          finalResponse.email,
          finalResponse.password,
          finalResponse.country
        ];
        const query =
          "INSERT INTO usuarios (name, lastname, nick, email, password, country ) VALUES ( ?, ?, ?, ?, ?, ?)";

        doQuery(query, params).then(results => {
          bodyResponse = JSON.stringify(
            wrapper.instanceStatusRegistered("OK", finalValidation)
          );
          console.log(bodyResponse);
          res.send(bodyResponse);
        });
...
/**
 * @description Methods that validates nick and email in database when the video is going to be registered
 * @param {} finalResponse - final error message
 */
async function executeRegisterValidations(finalResponse) {
  ...
  var validacionFinal = [];
  if (validationNick) validacionFinal.push("nick");
  if (validationEmail) validacionFinal.push("email");
